...
WebPulse is a small research prototype that:
...
6. Logs every session for review and demonstration.
...
- TTS engine — optional spoken output
...
```
webpulse/
  rppg/            # face detection, ROI extraction, signal filtering, HR/HRV estimation
  audio/            # pitch/energy extraction, valence scoring
  fusion/           # arousal+valence -> emotion label
  llm/              # prompt construction, API calls, response handling
  logging/          # session logger (CSV/JSON export)
  app.py            # main entry point / session runner
  SRS_WebPulse.md   # full SRS document
  README.md
  sessions/         # logged pilot session data (CSV/JSON, screen recordings)
```
